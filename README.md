...
- **Re-running a check** — the Checks tab's **Re-run** button is a *write* action. The classic `repo` scope already covers it; with a fine-grained PAT, also grant **Checks: Read and write** (and, for Actions-backed runs, **Actions: Read and write** — GitHub doesn't pin a single permission name for the re-run endpoint, so grant write access for checks/Actions and confirm in the token's settings). A read-only token still shows every check; only **Re-run** is gated, and it reports *"may not be re-runnable, or your token may lack write access"* if the token can't write.

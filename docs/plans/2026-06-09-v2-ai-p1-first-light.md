...
## PR #342 Merge Recalibration (2026-06-10)

After T1–T9 landed, this branch merged **PR #342 (`sync/main-into-V2`)** — origin/main + a large batch of landed V2 features. Merged base is green (build + ~1414 backend + 1663 FE tests). A per-task drift audit (8 agents vs the merged worktree) surfaced the corrections below. **The headline change: PR #283 / commit `d93b1d6c` flipped `AppConfig.Default` AiMode `Off` → `Preview`** (`new AiConfig(AiMode.Preview, AiConsentConfig.None, AiFeaturesConfig.AllOn)`); UiConfig gained `ContentScale "m"` (#135). T11, T12, T15 audited **clean** — execute as written.

- **T10 (medium).** (1) **Live cases must seed consent** or the selector resolves Noop → 204 (provider never called; tests would false-green): before calling the endpoint in the `200` and `503` cases, `factory.Services.GetRequiredService<AiConsentState>().Set(new AiConsentConfig(AiProviderIds.Claude, AiDisclosure.CurrentVersion, DateTimeOffset.UtcNow));` (add `using PRism.Core.Ai; using PRism.Core.Config;`). (2) **Subscriber setup via DI, not HTTP:** `POST /api/events/subscriptions` needs a live SSE conn (403 in tests). Replace `IActivePrCache` with `ConfigurableActivePrCache(subscribeAll: true/false)` via `WithWebHostBuilder` + `RemoveAll<IActivePrCache>()` (pattern: `PrRootCommentEndpointTests` line 317; `AllSubscribedActivePrCache` in `TestHelpers/SubmitEndpointFakes.cs`). (3) **Off case** must set `Mode = AiMode.Off` explicitly (no longer the default). (4) Step 3 "remove stale D111 TODO at lines 29-33" — those lines are `/ai/file-focus`'s comment (still valid; **do not remove**); only update the `/ai/summary` handler comment (lines 13-17).
- **T13 (low).** Plan accurate. Enforce **T12 before T13** (`getAiSummaryResult` is T12's deliverable). Default-flip makes the subscription gate load-bearing in prod → start `subscribed` state as `false`, flip to `true` only after the subscribe POST settles. `ActivePrUpdates` / `PrDetailContextValue` / `OverviewTab` shapes confirmed as the plan describes.
- **T14 (medium).** Component still pre-merge shape (single `summary` prop). (1) **Actively delete** the `SampleBadge` import + `<SampleBadge />` JSX (the live card has none). (2) Category div is currently **unconditional** → replace with the conditional `.chip` span. (3) In CSS, also delete the now-dead `[data-sample-badge] + *` adjacent-sibling rule alongside `.aiSummaryCategory`.
- **T16 (high).** (1) **Line numbers shifted** by #135's ContentScale slider: `aiModeShown` 57-58, no-op guard 66, SegmentedControl `value` 141. (2) **Three merged tests assert the OLD downgrade semantics and must be REPLACED, not extended:** test-file line 40 (`queryByRole('radio',{name:'Live'}).toBeNull()`), and lines 72-84 ("shows a live config as Preview" + "does not POST when already-shown Preview clicked on a live config"). Invert these to: Live IS present; a live config shows Live `aria-checked`; clicking Preview while Live DOES POST preview. (3) Add a no-op-guard test from the **Preview-default** start state. (4) Keep the ContentScale `FontSizeSlider` row (117-125) intact; mount `EgressConsentModal` outside the row divs.
- **T17 (medium).** Hard-blocked on T16 (Live radio). `ai-gating-sweep.spec.ts` has **no** disclosure/consent mocks — after T11 fixes the URLs, add `page.route` intercepts for the egress-disclosure GET (`{alreadyConsented:false|true, recipient, dataCategories}`) and the consent-record POST (`204`). `openSettings`/`selectAiMode` helpers **do not exist** — inline `page.goto('/settings/appearance')` + `getByRole('radio',{name:'Live'}).click()`. Mock fixture correctly stays at `aiMode:'off'` for isolation (no change).

### T18 Manual smoke (2026-06-11) — findings & additions

Smoke vs the **real `claude` CLI** passed A/B (consent modal → Accept → persisted), C (real summary renders in Live), E (provider-down → 503 error card). Two fixes + one feature landed during the smoke, beyond the 18 planned tasks:

- **Live-gate bug (fixed, commit `290fdcec`).** `useCapabilities` derived AllOn only for Preview (a PR3a #221 two-state rule written before Live existed); Live fell into AllOff, so `useAiGate('summary')` was false and the live summary card never fetched (the backend returned 200 — it was a FE gate, not a refresh bug). Fixed to tri-state: Off→AllOff, Preview→AllOn, **Live→only seams with a registered live impl (P1 = `summary`)**; other seams stay off in Live so a Preview-only sample card (SampleBadge self-hides outside Preview) never shows unbadged. Added `useCapabilities.test.ts` (the prior suite asserted Preview only).
- **`tsc -b` breakage from the merge (fixed, commit `460e72a4`).** 7 inline fixtures missing T13's required `subscribed` field; 2 new API tests on the old 2-arg `vi.fn<[Args],Ret>()` (the V2 merge bumped vitest to the 1-arg `vi.fn<(a)=>Ret>()` form). Vitest strips types, so `npm test` was green and only `npm run build`/`tsc -b` caught it.
- **AI interaction audit log (NEW feature, commit `3c730791`).** `IAiInteractionLog` + `JsonlAiInteractionLog`: one **metadata-only** JSONL line per AI seam invocation to `<dataDir>/logs/ai-interactions.log`, tagged with the triggering `component`; non-fatal; never prompt/response content. See spec §9. Surfaced **issue #379** — `claude-code` under-reports input tokens (no `CacheCreationInputTokens` on `LlmResult`; the CLI prompt-caches its harness), so token-spend visibility is misleading and `estimatedCostUsd` is the only reliable signal.

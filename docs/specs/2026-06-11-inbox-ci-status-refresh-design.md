...
Lever 1 narrows the bug but does not eliminate every instance of it. A *terminal* status (`Passing`/`Failing`) is still cached under its head SHA. If a user clicks **"Re-run failed jobs"** (or re-runs a workflow) without pushing, the head SHA is unchanged, so the **background poll alone** keeps serving the stale terminal value (`GitHubCiFailingDetector.cs:39`) and never re-probes — until the user clicks Refresh or the process restarts (empty cache).

A manual Refresh now **self-heals** this case: a forced reprobe (Lever 2) that observes a clean `Pending` *evicts* the stale terminal from the cache, so subsequent background sweeps re-probe (Lever 1 keeps `Pending` uncached) until CI goes terminal again, then re-cache. So one Refresh during a same-SHA re-run is enough — the dot tracks the live re-run from then on, rather than flipping back to the stale terminal on the next poll. (Without the eviction, Lever 1 only declines to *overwrite* the terminal, so the next non-forced sweep reads it and flips the dot back after a single render — the bug Copilot's review caught.)

The remaining gap is **fully-automatic** recovery: if the user never clicks Refresh, the background poll has no signal that a still-present same-SHA terminal needs re-validation. We **accept this gap**: same-SHA re-runs are uncommon relative to push-driven transitions, and Lever 2 gives a one-click fix that now sticks.

The durable, fully-automatic recovery (no manual Refresh needed) is a short TTL on terminal statuses (re-validate the same SHA after N minutes). It is **tracked in #361**, *not* tasked by #322: #322's cache acceptance criterion is "evict stale keys per tick (entries for PRs *absent* from the current snapshot are removed)" — eviction on absence, which never touches a still-present PR's same-SHA key. A re-validating TTL needs clock injection + per-entry timestamps that #322 does not introduce. #361 should land with or on top of #322's cache rework so the two don't fight. This is a *known, bounded* limitation — not the "buys nothing" framing an earlier draft used.
...
- `forceReprobe_nondegraded_pending_evicts_stale_cached_terminal`: prime the cache with `Passing`; a forced reprobe then observes a clean `Pending` (same-SHA re-run) → must *evict* the stale `Passing`, so the next *normal* sweep re-probes (two new HTTP) and returns the live `Pending` rather than the cached terminal; when the re-run finishes, a normal sweep advances back to `Passing`. Without eviction the normal sweep flips the dot back after one render (Copilot review). The mirror of the degraded test above: a clean forced `Pending` evicts, a degraded one does not.

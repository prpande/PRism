...
The sequencing trade is explicit, not implicit. **Post-amendment (2026-05-28)**, this rationale still holds: Phase 1 is deferred so Phase 2 (the README delta) becomes the only critical-path work between current state and tag. The polish-before-validate argument is unchanged — Phase 2 is still ~1 PR, ~1-2 days — and the v1 timeline shortens because Phase 1 implementation no longer sits in front of it. The original "Re-open if Phase 1 brainstorm extends by weeks" escape hatch (§ 6.1) is now resolved by the deferral itself.

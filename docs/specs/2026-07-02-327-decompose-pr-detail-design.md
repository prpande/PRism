...
- **Composer-location invalidation channel (required correctness counterpart).** Today the composer mounts/unmounts *because* `renderComposerForLine` is a fresh closure every render, busting every row memo. Once it's stable and the bodies are memoized, an `activeAnchor` change would alter **no** prop of the memoized bodies/rows — the composer would never open or close. And the composer is not the only such location: `renderComposerForLine` returns content for the composer's line **and** every line holding a new-inline optimistic placeholder, and these diverge (post at line A — placeholder stays — then open the composer at line B; the fallback-timer prune at A must still re-render A's row). So slice 2 threads an explicit **composite `activeComposerKey: string | null`** — the sorted join of the composer anchor's `file:line` plus every un-deduped new-inline placeholder's `file:line` (from `optimistic` entries with no `threadId`), else `null`; any membership change (open, move, post, prune) changes the string — through `UnifiedDiffBody`/`SplitDiffBody` (and into the row-level memo compare where a row's own key matches or stops matching), making the memo break exactly when the composer's location changes. In unified mode each `DiffLineRow` receives a **derived per-row boolean** (e.g. `isComposerLocation`, computed in the body by membership test), never the raw key — passing the raw key to all rows would break every row's memo on each composer move; the derived boolean keeps rows on their documented default shallow compare and re-renders exactly the joined/left rows. In split mode the composer row is emitted at body level (`emitWidgetAndComposerRows`), so the body-level memo break suffices (verified).
- **`replyContext` identity.** The `replyContext` memo's deps include `draftSession.session?.draftComments`/`draftReplies` and `optimisticByThread`, so its identity churns on every autosave refetch and reaches every unified-mode `DiffLineRow` — stabilizing only the two callbacks is not enough. Slice 2 splits `replyContext` into a stable-callbacks bag (latest-ref-backed) plus per-widget data resolved inside the widget. **Constraint:** the per-widget data must reach the widget through a channel that re-renders it when the data changes despite the row memos — React context or `useSyncExternalStore`, **not** a ref lookup (a ref read would pass the render-count test while silently going stale on cross-tab draft-reply arrivals, which `ExistingCommentWidget`'s draft hydration explicitly handles today). The channel should deliver **per-thread data slices whose identity is stable when that thread's slice is unchanged**, so an autosave refetch or a reply post re-renders only the affected thread's widget, not every row. Exact split settled at plan time against `ExistingCommentWidget`/`ThreadView`'s consumption (5 callbacks + per-thread data).
- **Render-count acceptance test** (extends the existing `rowMemo.perf` harness pattern): (a) typing in an open composer — spanning at least one autosave refetch in unified mode — does not re-render `DiffLineRow`s whose thread data is unchanged; (b) the inverse: clicking a line mounts the composer row, closing removes it (guards the `activeComposerKey` channel against staleness); (c) the reply-path inverse: posting a reply (an `optimisticByThread` change) and a refetch that alters a visible thread's draft-reply state each surface in the affected `ExistingCommentWidget` (guards the per-thread data channel against staleness).

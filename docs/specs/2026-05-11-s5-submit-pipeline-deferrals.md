...
revisions:
  ...
  - 2026-05-18: real-flow Task 16.5 surfaced stale-OID spec SSE/Reload-banner non-surfacing — deferred with two-hypothesis root-cause notes (see [Defer] entry).
  ...
  - 2026-05-18: real-flow Playwright e2e suite landed (3 of 4 scenarios) — closes [Defer] e2e Playwright test driving real usePrDetail → mark-viewed → submit → Finalize chain. See docs/specs/2026-05-18-real-flow-e2e-playwright-design.md + frontend/e2e/real/s5-real-*.spec.ts. Stale-OID scenario deferred separately (see new [Defer] entry in this file).
---
...
### [Resolved] Pre-existing Playwright e2e failures on `main` — verified on base commit `d6d15c2`
...
- **Date raised:** 2026-05-12 · **Resolved:** 2026-05-13
- **Reason (at the time):** Re-ran `cold-start.spec.ts` + `inbox.spec.ts` against the PR3 base commit (`d6d15c2`, before any S5-PR3 change) in a throwaway worktree — the **same** failures reproduce (`expect(getByRole('button', { name: /continue/i })).toBeDisabled()` at `/setup` → "element(s) not found"; analogous timeouts in `inbox`). So the failures are not a PR3 regression. PR3 itself is green on `dotnet test PRism.sln` (714 passed / 1 skip) and `frontend`'s `npm run lint` + `npm run build` + `vitest` (514 passed). This is the same e2e-suite instability the spec § 16 PR0b row / spec § 2.3 / the brainstorm-time PR0b deferral already owns ("Playwright multi-spec state-leak root-cause + fix"); PR0b is off the demo critical path.
- **Resolution:** PR0b set `workers: 1` in `playwright.config.ts`. Re-verified: full e2e suite under `CI=1 npx playwright test --project=prod` now passes 26 / skipped 1 (`no-browser.spec.ts`'s existing skip) / failed 0. The 8 pre-existing reds were all the same root family — single backend process + global fake state (`FakeReviewSubmitter` mutation counters, `FakeReviewBackingStore`, persisted `state.json`) — and serializing the workers closes all of them. True per-worker process isolation (backend per worker on its own port) is a larger structural change deferred to its own slice if/when e2e wall-clock becomes load-bearing.
...
## Implementation-time deferrals — S5 PR0b (Playwright multi-spec state-leak fix + un-fixme three S4 specs)

PR0b is the carve-out PR7 deferred (see `[Decision] PR7 ships defensively…` above). It removes the `--workers=1` interim requirement and un-fixme's the three S4 specs that PR7 had to leave behind. The fix premise — "concurrent Playwright workers against a single backend with global fake state pollute each other" — held; the `workers: 1` config change closed it directly. But un-fixme'ing the three S4 specs surfaced three *real* bugs that the carve-out had been hiding, all small, all in PR0b's spirit (make the un-fixme'd specs actually pass).

### [Decision] `workers: 1` baked into `playwright.config.ts`, not finer-grained per-worker isolation

- **Source:** PR0b execution (2026-05-13).
- **Affects:** `frontend/playwright.config.ts`; resolves `[Decision] The S5 e2e specs must run --workers=1` above.
- **Decision:** PR0b set `workers: 1` in `defineConfig`, with `fullyParallel: false` already present. Three options were weighed: (A) `workers: 1` always, (B) per-worker backend process via a worker fixture + private port, (C) hybrid — `workers: 1` only in CI + investigate stricter per-spec `/test/reset`. (A) chosen. Reasons: zero risk; directly closes the documented state-leak root cause; ~2× wall-clock cost is irrelevant at PoC scale (full e2e was ~40s under (A), single-digit-minute ceiling); throughput-cost-savings of (B) and (C) aren't load-bearing yet. The single-process FakeReviewSubmitter + FakeReviewBackingStore + state.json are the actual constraints; no amount of `/test/reset` hardening fixes that without per-worker process isolation. If e2e wall-clock becomes a real constraint post-S6, (B) can come back as its own slice. (C) is fiddly and not obviously cheaper than (B).
- **Revisit when:** E2E wall-clock becomes load-bearing — bring (B) back as a focused slice.

### [Decision] `/test/advance-head` re-seeds `IActivePrCache` symmetric to `/test/reset`

- **Source:** PR0b execution (2026-05-13) — surfaced when un-fixme'ing `s4-reconciliation-fires` + `s4-keep-anyway-survives-reload`.
- **Affects:** `PRism.Web/TestHooks/TestEndpoints.cs` — the `/test/advance-head` handler; original PR0b framing didn't anticipate this scope expansion.
- **Decision:** The handler now calls `IActivePrCache.Update(scenario, new ActivePrSnapshot(store.CurrentHeadSha, …))` after `store.AdvanceHead(...)`. Mirror of the same call in PR7's `/test/reset` hardening. Without it, `advanceHead()` updates `FakeReviewBackingStore.CurrentHeadSha` synchronously but the cache only catches up via the ~1s `ActivePrPoller` cadence — so a spec-immediate `POST /reload` reads a stale `cached.HeadSha`, mismatches `request.HeadSha`, and 409s with `reload-stale-head`. Both `s4-reconciliation-fires` and `s4-keep-anyway-survives-reload` use the exact `advanceHead → reload` pattern; both 409'd in their pre-PR0b run. Test code itself doesn't change — the synchronization moves to the right side (the test hook, not every caller). Two real specs unblocked.
- **Revisit when:** N/A — symmetric with `/test/reset`, intended end state. If a future test hook also mutates `store.CurrentHeadSha`, give it the same `IActivePrCache.Update` call.

### [Decision] `s4-multi-tab-consistency` rewritten to one `BrowserContext` + two pages (not two contexts)

- **Source:** PR0b execution (2026-05-13) — surfaced when un-fixme'ing the spec.
- **Affects:** `frontend/e2e/s4-multi-tab-consistency.spec.ts`.
- **Decision:** The original spec used `browser.newContext() × 2` to model "two tabs". But `BroadcastChannel` is scoped to one browsing context group, and Playwright's `BrowserContext` is "functionally a brand new browser profile" (per Playwright docs) — so two contexts have isolated browsing context groups and cannot share `BroadcastChannel` messages. The spec as written could never pass regardless of the underlying `useCrossTabPrPresence` logic. Production "two tabs" maps to one context with two pages (same browser profile, two tabs); the spec now uses `context.newPage() × 2` and a single cleanup `context.close()`. Comment in the file explains the constraint so the next contributor doesn't re-introduce the two-context shape.
- **Revisit when:** N/A — this is the only shape consistent with the production semantics being tested.

### [Decision] `s4-keep-anyway-survives-reload` regex `/override/i` replaced with `.locator('.chip.chip-override')`
...
**Status update (2026-05-18):** Resolved — scope expanded during brainstorm to a 4-scenario real-flow suite (happy-path + foreign-pending-review + lost-response-adoption + stale-commit-oid). See `docs/specs/2026-05-18-real-flow-e2e-playwright-design.md` for the design and `frontend/e2e/real/s5-real-*.spec.ts` for the implementation. The mark-viewed regression-net closes as a side-effect of the happy-path spec, which catches the original PR#55 bug class through the real FE wire-up. **Three of four scenarios are passing on first attempt;** the stale-commit-oid spec is `test.skip`ed pending further investigation — see the new `[Defer] Real-flow stale-OID spec — SSE/Reload-banner non-surfacing after createCommitOnBranch` entry elsewhere in this file.
...
## Implementation-time deferrals — real-flow e2e Task 16.5 (2026-05-18)

### [Defer] Real-flow stale-OID spec — SSE/Reload-banner non-surfacing after createCommitOnBranch

- **Source:** Real-flow e2e suite (Task 16.5, 2026-05-18 implementation run)
- **Severity:** P3 — three of four real-flow specs pass and catch the bug class the suite was built for (PR#55 mark-viewed regression-net, DiffSide casing, foreign-pending HTTP 500). The skipped spec exercises the stale-commit-OID + SSE-poller-banner pipeline against live GitHub.
- **Date:** 2026-05-18
- **Reason:** `s5-real-stale-commit-oid.spec.ts` waits for a Reload-PR banner to appear after `createCommitOnBranch` advances the fixture branch head. The banner does not surface within a 150s budget on `prpande/prism-sandbox`. Two plausible root causes (one infrastructural, one production):
  1. GitHub PR record propagation lag (REST `repos/.../pulls/{n}.head.sha` update asynchronicity) — possibly longer than 150s on free-tier sandboxes.
  2. `BannerRefresh.formatMessage` returns `''` when `headShaChanged=false` AND `commentCountDelta<=0`, so `if (!message) return null` renders no banner. `ActivePrPoller` emits `ActivePrUpdated(HeadShaChanged=false, CommentCountChanged=false, NewHeadSha=null)` on first-poll-after-subscribe regardless of whether the head actually changed since the last subscription cycle. If the spec's first poll for the fixture PR lands AFTER `advanceHead`, the head delta is silently lost.
- **Revisit when:**
  - A backend log capture (Playwright `webServer.stdout` redirected to file, or `ASPNETCORE_LOGGING__LOGLEVEL__PRism.Core.PrDetail.ActivePrPoller=Debug` forwarded) confirms which of the two causes is actually firing on a failing run.
  - If cause (2), the fix is one of:
    - `ActivePrPoller`: on first-poll-after-subscribe, fetch the prior head from `IActivePrCache` (if cached from a previous subscription cycle) before computing `headChanged`. If no cache entry, treat the first poll as `HeadShaChanged=null`-bearing (current behavior), and require a SECOND poll for a true head-diff.
    - `BannerRefresh`: render a generic "PR has been updated — Reload to view" message when `firstPoll && !headShaChanged && !commentChanged` AND the fetched snapshot's head differs from what `usePrDetail` initially loaded.
  - The spec stays `.skip`ed in `frontend/e2e/real/s5-real-stale-commit-oid.spec.ts` until the fix lands.
- **Original finding evidence:** Real-flow Task 16.5 deterministic same-spec failures across multiple wait-budget tries (30s, 60s, 90s, 150s — all timed out at `expect(reloadBanner).toBeVisible(...)`). Investigation surfaced via code reading of `PRism.Core/PrDetail/ActivePrPoller.cs:115-135` + `frontend/src/components/PrDetail/BannerRefresh.tsx:43-62`.

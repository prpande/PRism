...
```
**Backend capability lights up automatically:** `AiCapabilityResolver` reads the same live `realSeams` dictionary, so once `IHunkAnnotator` is registered there, `HunkAnnotations` reports capable in Live (given availability + consent + the `hunkAnnotations` feature enabled). `AiSeamFeatureKeys` already maps `IHunkAnnotator → "hunkAnnotations"`, and `AiFeaturesConfig.AllOn` already includes it. `AiSeamWarmup` already forces selector construction at startup.

**FE Live gate — ONE required edit (corrected post-preflight 2026-06-14):** the FE does NOT yet consume the backend's per-flag `/api/capabilities` wire (D112 deferral); it derives capabilities locally in `useCapabilities.ts` from a hardcoded `LIVE_CAPABILITIES` set. That set lists only `summary` + `fileFocus` as live, so `useAiGate('hunkAnnotations')` would return **false** in Live and `DiffPane` would never call the endpoint — the backend would ship **dark**. **This slice MUST add `hunkAnnotations: true` to `LIVE_CAPABILITIES`** (and update `useCapabilities.test.ts`). (The original spec wrongly stated "no FE `useCapabilities` edit needed"; the preflight adversarial review caught it.)

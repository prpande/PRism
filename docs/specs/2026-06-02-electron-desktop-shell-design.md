...
1. **Verify + pin the launch contract (mostly verification, not new code).** Confirm `--no-browser` is parsed, the port is reported to stdout in a stable parseable form, and `--dataDir` binds via the config provider's `DataDir` key (it is *not* a bespoke flag). **Add a test pinning the `--dataDir` → `DataDir` contract** so an SDK default-config change can't silently break the sidecar's data directory. The health endpoint already exists — `GET /api/health`, auth-exempt — so **no `/healthz` is added** (the earlier "add if needed" option is moot).
...
- Terminal state of this brainstorm: invoke `superpowers:writing-plans` on this spec to produce the implementation plan (backend changes on `main` → `desktop/` scaffold → packaging → CI matrix → e2e → `TESTING.md`).

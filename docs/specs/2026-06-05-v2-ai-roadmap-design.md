...
*Items 1–2 are BLOCKING with owners still TBD. The spec is approvable as a design, but **P0 cannot start until both owners are named** — assigning them is a P0-kickoff precondition, not a P0 deliverable.*
...
2. **Post-June-15 empirical credit-failure probe** *(BLOCKING P0 EXIT — owner: TBD, deadline ≤ P1a)* — who, on a real Pro/Max subscription that has done the one-time human credit opt-in, captures the exact stderr/exit/JSON signatures for the not-opted-in and exhausted states. **Capturing the *exhausted* signature is destructive and non-repeatable** (it drains a real finite monthly credit that cannot be undone within the month). Resolution: the probe either deliberately burns a low-tier (Pro) credit to capture it, or — if exhaustion cannot be induced by the deadline — P0 exits with the *not-opted-in* signature captured (the hard blocker) and a **defensive unknown-bucket classifier for exhaustion**, backfilled when exhaustion next occurs naturally. No live call reaches real reviewers (P1) against an unverified not-opted-in classifier.

...
**New Settings → AI section (P0):** detect Claude Code, show login + credit status, surface the 4 disabled states with specific guidance, and (when built) configure the API-key fallback. This is where the user "configures AI after setup." It also **discloses third-party data egress** — that enabling Live mode transmits PR content (diffs, titles, descriptions, comments; metadata for the inbox batch) to **the configured provider's recipient** (named by the descriptor's recipient-identity string, e.g. "Anthropic, via the Claude Code CLI") — enumerates the data categories sent per call type (per-PR vs. inbox batch), and gates the **first** Live call behind an explicit consent acknowledgement. **Consent is per-provider:** switching the active provider invalidates the prior consent and requires a fresh disclosure naming the new recipient; Settings → AI always shows the currently-consented provider. **Per-feature user-enablement toggles** also live here. **Model (decided 2026-06-09, during P1 First-Light planning):** beyond *system capability* ("can the provider do it"), each of the nine AI features carries a user-controlled `userEnabled` flag so a cost-conscious user can switch off any feature they don't find worth the tokens. The effective gate is `effective(feature, mode) = mode≠off && userEnabled(feature) && capableForMode(feature, mode)`. It is **backend-enforced** — a disabled feature resolves to `Noop` (204, no token spend), never a frontend-only hide — because the N-PR inbox fan-out features (enrichment, ranking) are the real spenders. Defaults are all-on (behavior unchanged until the user opts out). **The backend seam (storage `ai.features.<key>` + `AiFeatureState` holder + the gate term) lands in P1 First-Light with Summary as the first consumer**, so P2's inbox seams inherit the gate with no retrofit; the per-feature **toggle UI** and the FE per-flag wire (D112) land with this Settings → AI pane. Inbox categorization is the highest-value toggle (see §3.2 cost-control decisions); merged/closed PRs are never enriched except on explicit request.
...
- **N=3 preconditions** (the credit regime is days old at authoring time): each external validator must have a qualifying subscription, have completed the one-time human credit opt-in (§2.2), and have non-exhausted credit before the evaluation window; if credit exhausts mid-evaluation the run restarts rather than counting partial signal.
  ...
  > **Note (2026-06-13, P1-2 brainstorm decision):** For the current solo PoC the N=3 re-sample + ≥8-reference golden-set gate is **dropped**. It is overweight for single-user dogfooding. The light multi-PR sample described in the P1-2 spec (§1/§14, [`docs/specs/2026-06-13-v2-ai-p1-2-file-focus-design.md`](2026-06-13-v2-ai-p1-2-file-focus-design.md)) is the sole bar for the file-focus slice. No downstream structured seam is blocked on a calibration gate while PRism is a PoC. Reinstating a structured-reliability calibration gate is a fresh decision post-PoC, not a re-enable of this bullet.

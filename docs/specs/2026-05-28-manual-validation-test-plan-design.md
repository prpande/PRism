...
| **`<dataDir>`** | PRism's data directory. Production: `%LOCALAPPDATA%\PRism` (Windows) / `${XDG_DATA_HOME:-~/.local/share}/PRism` (macOS + Linux). Validation suite: per-run temp dir managed by the suite's `webServer` block |

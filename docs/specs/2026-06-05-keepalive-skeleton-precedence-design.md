...
This is the minimal correct change at the right altitude: it aligns the render with `usePrDetail`'s existing data-preservation intent rather than special-casing the re-activation path or plumbing scroll restoration into the inner containers. Because the subtree is no longer unmounted, the inner diff/tree scroll positions survive naturally (a `hidden`/re-shown element retains its descendants' `scrollTop`). The inner FilesTab tree skeleton (`FilesTab.tsx`, gated on `diff.showSkeleton`) keeps the same skeleton-over-content pattern and is **left untouched on purpose**: `useFileDiff` is range-keyed (no `reloadCounter` dependency), so it does not refetch on re-activation and that skeleton never flips on this path — it only shows on a genuine iteration/commit-range change, where a tree skeleton is correct because the file row set is genuinely unknown.

### Loading affordance and failure behavior (decided)

- **Silent in-place update is intentional.** On a same-PR background reload (re-activation freshness or the manual Reload button), there is deliberately **no skeleton, spinner, or `aria-busy`** while `data` is present. The whole point of keep-alive is an instant return; any loading chrome on tab-return would re-introduce the flash this fix removes. The content stays on screen and fully interactive during the refetch and updates in place when it resolves — standard SPA behavior. `aria-busy` is intentionally *not* set, because the content region remains *available* (not being replaced); marking it busy would mislead assistive tech. (Pre-fix, the skeleton's `aria-busy` fired but also destroyed the user's place — strictly worse for everyone, SR users included.) A fresh-data "updated" announcement is a possible future enhancement, out of scope here.
- **Failed background reload.** `usePrDetail` keeps the prior `data` and sets `error` on a rejected same-PR reload (it clears `data` only on PR-navigation), and `useDelayedLoading` drops `showSkeleton` once `isLoading` clears — so there is **no stuck skeleton**. With the `!data` gate the stale content stays mounted and the existing error banner (`PrDetailView.tsx`, `error && <div role="alert">…`) renders alongside it. This is the same contract the existing OQ6 freshness test already locks; the fix does not change it.

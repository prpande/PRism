...
- **Resolved (#142, 2026-06-06) — NOT REPRODUCIBLE, no dedup built.** The premise (server resends buffered events on reconnect) does not hold against the current live-fanout SSE design: `SseChannel.OnActivePrUpdated` writes only to currently-connected subscribers with no replay queue; `SseEventProjection.Project` emits no `id:` line and the server reads no `Last-Event-ID`, so a reconnect never replays history; and `ActivePrPoller` tracks `LastCommentCount` server-side, so every delta it emits is a genuine change (the first poll after any re-subscribe is `firstPoll` → delta 0). The deferral's own revisit gate (Last-Event-ID resume **and** at-most-once delivery) is unmet. Building server event-ids + client dedup would be infrastructure for a bug that cannot currently occur (YAGNI). Locked in with a regression test — `useActivePrUpdates.test.tsx` › "does not double-count commentCountDelta across an SSE reconnect" — asserting a reconnect alone never inflates the accumulated count while genuinely new post-reconnect events still accumulate. Re-open only if a Last-Event-ID resume protocol is added to the SSE layer.
...
- **Resolved (#142, 2026-06-06) — fix candidate (c), client-only.** `useActivePrUpdates` now captures the in-flight subscribe POST promise (`lastSubscribePost`) and the cleanup chains the unsubscribe DELETE behind it (`lastSubscribePost.catch().then(() => apiClient.delete(...))`), so the server always observes POST→DELETE and never lands a dangling subscription. No server change needed — sequence numbers (a) and `sendBeacon` (b) were rejected as over-engineered for a frontend-resolvable ordering issue; `sendBeacon` also only covers tab-unload, not the dominant SPA PR-to-PR navigation unmount. Covered by `useActivePrUpdates.test.tsx` › "delays the unsubscribe DELETE until the in-flight subscribe POST resolves" (asserts the DELETE is not issued while the POST is pending, and POST resolves strictly before DELETE).

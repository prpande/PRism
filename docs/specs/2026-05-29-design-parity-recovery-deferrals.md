...
**Status:** DEFER-TO-V1.X.
**Update (#221, 2026-06-06):** To fix AI-preview toggle reactivity, `useCapabilities` now **derives** `AllOn`/`AllOff` from the shared `aiPreview` preference instead of fetching `/api/capabilities` (which only mirrored `aiPreview` anyway). Two consequences for this deferral: (1) the frontend no longer calls `/api/capabilities` (the backend endpoint is retained but unused by the SPA); (2) decoupling is **no longer purely backend-only** — the reopener below now also requires swapping `useCapabilities` back to an independent reactive source (the shared-store / `CapabilitiesContext` mirrored from #143). `useAiGate`'s two-factor `capabilities[key] && aiPreview` shape was deliberately left unchanged to keep that swap localized. See `docs/specs/2026-06-06-ai-toggle-reactivity-design.md`.
**Reopener:** Backend capability decoupling lands. When `AiCapabilities` stops mirroring `AiPreviewState.IsOn`, re-evaluate the two key choices documented above (either confirm the coupling-by-name or add dedicated `askAi` / `activityRail` keys) **and** restore an independent reactive `useCapabilities` source (per the #221 update above).
**Cross-refs:** § 3.1 coupling caveat; § 4.2 + § 4.6 key-choice rationale; D109 + D111 (real-AI swap pulls capability decoupling forward); #221 (frontend derive).

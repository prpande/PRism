...
- `frontend/src/hooks/useEffectiveLocation.ts` — resolves the *effective* app path (collapses Settings/Help/Feedback modal URLs to their background), already consumed by `PrTabHost`/`PrTabStrip`.
- `frontend/src/pages/InboxPage.tsx` — the Inbox view, unchanged by this slice; its existing `useState` (filter, sort, onboarding-dismissed) and descendants' state (`ActivityRail`'s `showBots`) are what keep-alive preserves.
...
- **`InboxPage` is near-unchanged.** Filter/sort/onboarding/rail state are already React state; staying mounted preserves them for free. No `useParams` threading (the Inbox has no route params), so none of the child-prop surgery the PR-detail slice needed applies. The **one** addition is a single `active` prop (mirroring `PrDetailView`'s `active`) used solely to gate the page's `Modal`-based dialogs (`AiOnboardingDialog`, the load-error `ErrorModal`) so a hidden-but-mounted Inbox does not leak document-level keydown handlers (§4). Everything else in `InboxPage` is untouched.
- **No focus-refetch.** Unlike PR-detail (§4.1 of the parent), the Inbox already self-refreshes while mounted: `useInboxUpdates` listens on the app-wide `useEventSource()` stream and reloads (debounced) on every `inbox-updated` frame, so a hidden Inbox stays fresh and returning shows current data. Adding a focus-refetch would be redundant **and** would shift content under the restored scroll. Explicitly omitted.
...
1. **Reads `useEffectiveLocation()`** and computes `onInbox = pathname === '/'`. Using the *effective* path (not the raw `useLocation`) means an open Settings/Help/Feedback modal — whose live URL is `/settings/*` etc. but whose background is the Inbox — keeps the Inbox visible behind the scrim, matching how `PrTabHost`/`PrTabStrip` already behave. When the modal's background is a PR, the effective path is the PR route and the Inbox is hidden.

...
**State (provider-held).** Provider holds `{ isOpen: boolean, threads: Map<prRefKey, ChatThread> }` where `ChatThread = { messages: ChatMessage[], input: string, pendingAiReply: boolean, cycleIndex: number }` and `ChatMessage = { role: 'user' | 'ai', body: string, ts: number }`. Threads are per-PR (`prRefKey` = `${owner}/${repo}#${number}`); selecting a different PR shows that PR's thread or an empty one. Threads persist across drawer close/reopen on the same PR AND across PR-to-PR navigation. Threads are wiped only by: hard page reload, `prism-identity-changed` event (matches `OpenTabsContext` pattern from PR7).

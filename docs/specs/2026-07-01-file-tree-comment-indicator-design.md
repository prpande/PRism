...
- **No border/seam, and full-row highlight (owner direction 2026-07-01).** At rest the rail shares the tree's `--surface-1`, so with no divider it reads as one continuous surface. Beyond rest, the row's **hover (`--surface-3`) and selected (`color-mix(accent-soft 40%, surface-2)`) backgrounds extend across ALL four columns** — comment rail, name, AI dot, checkbox — so hovering or selecting a row paints one continuous bar edge-to-edge, not just the name cell. This **also fixes the pre-existing AI + checkbox columns**, which today keep `--surface-1` while only the name cell highlights (a visible seam this change removes). Scope note: this touches the two existing gutters as well as the new one.

  *Mechanism* (the four columns are separate DOM siblings, so `:hover` on one cannot style the others):
  - **Selected** — no new state. `selectedPath` is already a `FileTree` prop in scope in the render body where every column maps over `rows`; each slot derives `selected = row.node.path === selectedPath` inline and paints the selected wash. (Feasibility confirmed `selectedPath` reaches the AI/check `rows.map` at `FileTree.tsx:271,292`.)
  - **Hover** — a single delegated `onMouseOver` + `onMouseLeave` on `.fileTreeBody` resolves the hovered row and sets a lifted `hoveredPath` state; all four columns paint the slot whose row matches. **The row-resolution attribute must be on EVERY column's per-row slot** — `data-row-path` (files) / `data-row-key` (dirs) on the comment slot, name cell, AI slot, and check slot alike — so `closest('[data-row-path],[data-row-key]')` resolves the same row no matter which column the pointer is over. Today only the name cell carries `data-path` (`FileTree.tsx:359`); mirroring it to only the name cell would make gutter hovers resolve to null and drop the highlight (flicker) exactly on the gutters this change exists to fix. `onMouseOver` whose target resolves to no row leaves `hoveredPath` unchanged (only `onMouseLeave` on the body clears it). One re-render per row-to-row transition — negligible for a bounded, lightweight file tree (contrast the diff, where #670's memoization mattered).
  - **Selected + hover precedence — selected WINS.** A row can be both selected and hovered (hovering the currently-open file is the most common interaction here). Model the two as **independent flags** (`data-selected` + `data-hovered`, not one mutually-exclusive enum) and give the CSS an explicit precedence: when both are set, the row keeps its **selected** wash (`color-mix(accent-soft 40%, surface-2)`) — hover produces no visual change on an already-selected row. A single enum that lets hover overwrite selected would flicker the open file's selected affordance away while the cursor rests on it. (v1 uses no distinct "selected+hover" third background; a combined treatment is a possible later refinement, out of scope.)
  - **Directory rows** participate in hover (they are never selectable — no selected state). Their AI/check/comment slots are empty, so a hovered folder paints a continuous bar across those empty gutters. This is **intentional** — one edge-to-edge highlighted row, matching how file trees (VS Code, GitHub) highlight the whole row including empty gutter areas. Scoping dir hover to the name column only would reintroduce the very seam this change removes, so we do not.
  - One CSS rule set paints all four columns identically per state so the bar is a single color with no step at a column boundary. The name cell's background lives inside `.fileTreeScroll` (moves with `translateX`) but the row is full-width (`width:100%` of the `max-content` inner), so it always fills the visible name area — continuous with the static gutters, with no pixel gap (`.fileTreeBody` has no flex `gap`; verified by feasibility).
...
9. Full-row highlight: selecting a file marks the selected state on the comment, name, AI, and checkbox slots for that row (not just the name cell); hovering a row (delegated on `.fileTreeBody`, resolving via the per-slot `data-row-path`/`data-row-key`) sets `hoveredPath` and marks the same four slots; leaving clears it. **Precedence:** hovering the currently-**selected** row keeps the selected wash (selected wins — `data-selected` present, hover adds no visual change). **Gutter-hover resolution:** a pointer over the AI/checkbox/comment slot (not the name cell) still resolves and highlights the correct row (guards the all-columns attribute). **Directory hover:** a hovered dir row marks all its column slots (continuous bar over empty gutters) and never enters selected state. Regression guard: unrelated rows carry neither state.

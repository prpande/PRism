...
- **No border/seam, and full-row highlight (owner direction 2026-07-01).** At rest the rail shares the tree's `--surface-1`, so with no divider it reads as one continuous surface. Beyond rest, the row's **hover (`--surface-3`) and selected (`color-mix(accent-soft 40%, surface-2)`) backgrounds extend across ALL four columns** — comment rail, name, AI dot, checkbox — so hovering or selecting a row paints one continuous bar edge-to-edge, not just the name cell. This **also fixes the pre-existing AI + checkbox columns**, which today keep `--surface-1` while only the name cell highlights (a visible seam this change removes). Scope note: this touches the two existing gutters as well as the new one.
  ...
  *Mechanism* (the four columns are separate DOM siblings, so `:hover` on one cannot style the others):
  - **Selected** — no new state. `selectedPath` is already a `FileTree` prop; each column's slot paints the selected wash when its row's path equals `selectedPath`.
  - **Hover** — a single delegated `onMouseOver` + `onMouseLeave` on `.fileTreeBody` resolves the hovered row from a `data-row-path` (files) / `data-row-key` (dirs) attribute and sets a lifted `hoveredPath` state; all four columns paint the slot whose row matches. One re-render per row-to-row hover transition — negligible for a bounded, lightweight file tree (contrast the diff, where #670's memoization mattered). Directory rows participate in hover (they are not selectable, so no selected state for them).
  - Slots express state via a shared `data-row-state="hover"|"selected"` attribute (or equivalent class); one CSS rule set paints all four columns identically so the bar is a single color with no step at a column boundary. The name cell's background lives inside `.fileTreeScroll` (moves with `translateX`) but the row is full-width, so it always fills the visible name area — continuous with the static gutters.
...
9. Full-row highlight: selecting a file marks the selected state on the comment, name, AI, and checkbox slots for that row (not just the name cell); hovering a row (delegated on `.fileTreeBody`) sets `hoveredPath` and marks the same four slots; leaving clears it; a directory row hovers across its columns but never enters selected state. Regression guard: unrelated rows carry neither state.

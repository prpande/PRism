...
- **Egress-consent sequencing (cross-PR).** See §2 — PR3b's consent gate must land before any P1 real Live seam is registered, or a hand-edited Live config could egress PR content without consent. Bounded today by the empty real-seam bag and the locally-bound sidecar (host/origin/session middleware limits the surface to the local user). The enforcement artifact is a **GitHub tracking issue** (§2), referenced from the PR3b spec; the durable server-side guard is PR3b's to build.

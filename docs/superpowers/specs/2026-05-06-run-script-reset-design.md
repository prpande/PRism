...
- If `<dataDir>\state.json` does not exist, create it with the v1 default shape but `lastConfiguredGithubHost = "https://prism-reset-stub.invalid"`. Default shape: `{ "version": 1, "reviewSessions": {}, "aiState": { "repoCloneMap": {}, "workspaceMtimeAtLastEnumeration": null }, "lastConfiguredGithubHost": "https://prism-reset-stub.invalid" }`. This matches `AppState.Empty` from [`PRism.Core/State/AppState.cs`](../../../PRism.Core/State/AppState.cs) with the sentinel substituted in.
...
- `.\run.ps1 -Reset Auth` does **not** touch the token cache; it sets `state.json.lastConfiguredGithubHost = "https://prism-reset-stub.invalid"` so the next launch's `/api/auth/state` probe reports a host mismatch and surfaces the host-change-resolution modal. All other JSON values in `state.json` are preserved unchanged (formatting may reformat). If `state.json` is missing, it is created with the v1 default shape (`AppState.Empty`) plus the sentinel host. If `state.json` exists but is malformed, the script aborts with a clear error and does not overwrite it.

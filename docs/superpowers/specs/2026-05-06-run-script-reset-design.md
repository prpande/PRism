...
- **Auth-adjacent** — re-test the host-change-modal flow (which fires when `state.json.lastConfiguredGithubHost` no longer matches `config.github.host` at startup).
...
| `Auth` | Token cache (as above) **plus** the `lastConfiguredGithubHost` field is set to `null` in `state.json`. All other JSON values in `state.json` are preserved unchanged (whitespace and indentation may reformat — the host does not depend on file formatting, only on the JSON contents). | Drafts, pendingReviewIds, etc. | Note: per `AuthEndpoints.MapAuth`'s `/api/auth/state` handler, host-mismatch is only reported when `LastConfiguredGithubHost is not null`; a `null` value does *not* trigger the host-change modal on its own. `Auth` mode therefore tests the "fresh-state-with-existing-config" path; to additionally trigger the host-change modal, also change `config.github.host` to a different URL. |
...
- If `<dataDir>\state.json` does not exist, the token-deletion still runs, then the script proceeds. The C# host will create a fresh `state.json` at startup with `LastConfiguredGithubHost: null` already, so the absence-of-file case is the same as the cleared-field case from the host's perspective.
...
- If `<dataDir>\state.json` is parseable, set `lastConfiguredGithubHost` to `$null` and write the result back to `<dataDir>\state.json` as UTF-8 without BOM. The round-trip uses PowerShell's `ConvertFrom-Json` / `ConvertTo-Json -Depth 10`; this preserves all string and number values verbatim (existing kebab-case enum strings on disk round-trip as strings, no enum conversion happens) but **may reformat whitespace and indentation**. The host does not depend on `state.json` formatting, only on its JSON contents, so this is harmless. To avoid a half-written file on power loss between read and write, write to `<dataDir>\state.json.tmp` first, then replace the original via `[System.IO.File]::Move($tmp, $real, $true)` (which uses `MoveFileEx` with replace-existing on Windows — the closest practical primitive to atomic).
...
|---|---|---|---|
| Windows | Correct — DPAPI-encrypted file at `<dataDir>\PRism.tokens.cache` is the only persistence | Correct — single `state.json` file mutation | Correct — `<dataDir>` is the only place PRism writes |
| macOS | **Cosmetic only** — secret lives in Keychain (`com.prism.tokens` / `github-pat`), not the file | Correct — `state.json` is the same on macOS | Correct — `<dataDir>` is the same on macOS |
| Linux | **Cosmetic only** — secret lives in libsecret schema `com.prism.tokens` | Correct | Correct |
...
4. **`Auth` mode field surgery.** With a `state.json` that has `lastConfiguredGithubHost: "https://github.com"` and a non-empty `reviewSessions`, run `.\run.ps1 -Reset Auth`. Verify `lastConfiguredGithubHost` is now `null` (use `Get-Content state.json | ConvertFrom-Json | Select-Object -Expand lastConfiguredGithubHost` and confirm it returns `$null`) and that `reviewSessions` parses to the same object shape as before (compare `ConvertFrom-Json` output, not raw bytes — formatting may differ). Verify the host-change modal is reachable on next launch (set `config.github.host` to a non-default value first to trigger it).
5. **`Auth` mode malformed-JSON abort.** Truncate `state.json` to a half-written JSON. Run `.\run.ps1 -Reset Auth`. Verify the script aborts with a non-zero exit, prints a clear error, and does NOT overwrite the file.
...
- `.\run.ps1 -Reset Auth` removes the token cache as above, plus surgically sets `state.json.lastConfiguredGithubHost = null`. All other JSON values in `state.json` are preserved unchanged (formatting may reformat). Aborts with a clear error if `state.json` is malformed; does not overwrite it.

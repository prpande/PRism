...
- **S3 row** — fill in spec status (`Shipped — superpowers/specs/2026-05-06-s3-pr-detail-read-design.md`) on slice completion. Add: "Adds `ViewedFiles` field to `ReviewSessionState` + a single inline `MigrateV1ToV2` helper. Closes the spec-mandated `X-PRism-Session` middleware gap (security baseline; see § 8 design contract). ADR-S4-1 (`PrSessionsState` wrap) and ADR-S4-2 (migration framework) remain gated to S4."

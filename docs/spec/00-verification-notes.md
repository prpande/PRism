...
| M22 | `react-markdown` v9 strict-mode sanitization is on by default and tight enough for untrusted PR content | **PARTIAL** (claim was wrong: posture is *escaping by default*, not sanitization; XSS surface is still closed but for the wrong reason) | Escaping regression test already shipped at `frontend/__tests__/MarkdownRenderer.sanitization.test.tsx` — pins the current posture. Remaining: document the `rehype-raw` + `rehype-sanitize` pairing rule as the prerequisite for any future raw-HTML feature, and consider a CI lint that rejects `rehype-raw` imports without a sibling `rehype-sanitize`. |
...
- **`react-markdown` v9 escapes raw HTML by default; it does NOT use `rehype-sanitize` unless you pass it in `rehypePlugins`.** Per the official docs, `rehype-sanitize` is *recommended* but not applied automatically. To even feed HTML through a sanitizer, you also have to install `rehype-raw` so the parser surfaces HTML nodes (instead of escaping them as text). The XSS posture for the default configuration is therefore "all raw HTML in the markdown source becomes literal text in the DOM" — not "HTML is parsed and a sanitizer enforces an allowlist."
- **The PoC's `MarkdownRenderer.tsx` ships with the default configuration** — `react-markdown` + `remark-gfm` only, no `rehype-raw` and no `rehype-sanitize`. So raw HTML in PR bodies, comment bodies, and viewable `.md` files is escaped to text, not rendered. The XSS surface today is closed *by escaping*, not by sanitization, and the earlier wording in this note had the rationale wrong.
- Mermaid fences are dispatched by the custom `code` component renderer **before** any HTML processing — the dispatcher consumes the fence content as raw text and hands it to `mermaid` for SVG generation; the SVG is then inserted via React directly. Since no `rehype-sanitize` is in the pipeline, there is no sanitizer to bypass; the carve-out is real but the framing is "Mermaid produces its own SVG, which the React renderer trusts" — not "Mermaid bypasses the sanitizer."

### Implication for Wave 2

1. **Escaping regression test — already shipped at `frontend/__tests__/MarkdownRenderer.sanitization.test.tsx`.** The test asserts that `<script>` payloads render as escaped text (no real `<script>` element), `[text](javascript:...)` links are stripped by the explicit `urlTransform` allowlist (https?/mailto only) including HTML-entity-obfuscated variants, and reference-style `javascript:` links are also stripped. The test is the load-bearing trip-wire that guarantees a future "loosen rendering to support feature X" change cannot silently flip the posture from "escaped" to "parsed-but-not-sanitized." When extending the rendering pipeline (e.g., adding `rehype-raw`), the test must be amended to assert the new posture explicitly.
2. **`rehype-raw` + `rehype-sanitize` pairing as the prerequisite for any future raw-HTML feature (still doc-only — not enforced in code).** Anyone wanting to support `<details>`/`<summary>` (or any other element teams' PR templates use) must add **both** `rehype-raw` (to surface HTML nodes) **and** `rehype-sanitize` (to filter them) in the same change. Adding only `rehype-raw` would parse arbitrary HTML and ship it to the DOM — an immediate XSS surface. The pairing rule lives in this verification note today; landing it in `MarkdownRenderer.tsx` as a comment-anchored invariant + a CI-enforced prohibition (e.g. an eslint rule rejecting `import 'rehype-raw'` without a sibling `import 'rehype-sanitize'`) is the next defense layer.

The shipped escaping regression test pins the current posture; the pairing rule is the doc-side defense against silent posture changes. M22 records both so future contributors know the actual posture and the prerequisite for changing it.
...
**Verified 2026-05-12** (S5 PR0a, Task 6): **marker preserved verbatim in all three cases.** The spec stands as written; the `<!-- prism:client-id:<id> -->` marker is the canonical idempotency key for the lost-response window. No fallback needed.

Test ran against sandbox PR `prpande/prism-sandbox#1`. Procedure: `addPullRequestReview` (no event) → three `addPullRequestReviewThread` calls, one per marker body, anchored on consecutive lines of `Calc.cs`, side RIGHT → query `pullRequest.reviews(states: [PENDING], first: 1).nodes[0].comments(first: 10).nodes.body` (note: `PullRequestReview` has no `threads` field — the plan's draft query used a nonexistent field; the comment list on the pending review is the correct read; cross-checked against `pullRequest.reviewThreads`, which agreed) → `deletePullRequestReview` cleanup.

Observed `body` fields, verbatim (newlines shown as `\n`):

- Case 1 (marker only): `<!-- prism:client-id:c7-test-1 -->` → marker preserved? **YES**
- Case 2 (marker as footer): `This is a test comment for C7 verification.\n\n<!-- prism:client-id:c7-test-2 -->` → marker preserved? **YES**
- Case 3 (marker after a fenced code block, marker outside the fence): ` ```ts\nconst x = 1;\n```\n\n<!-- prism:client-id:c7-test-3 --> ` → marker preserved? **YES**

Bodies round-tripped byte-identical (no line-ending, whitespace, or entity normalization observed).

**Implication for PR2:** Spec's marker-based adoption (§ 5.2 step 3) ships as written. PR7's `s5-submit-lost-response-adoption.spec.ts` stays as planned (not the body-normalization-parity variant). The documented fallback (client-side body normalization parity) is not built.
...
**Verified 2026-05-12** (S5 PR0a, Task 7): **GraphQL accepted the empty-threads Comment finalize.** `addPullRequestReview` (no event) → no `addPullRequestReviewThread` calls → `submitPullRequestReview(event: COMMENT, body: "<non-empty>")` returned success with `state: COMMENTED`. The review posted to github.com (`https://github.com/prpande/prism-sandbox/pull/1#pullrequestreview-4270367471`, sandbox PR — no longer relevant; URL retained for traceability). The spec's empty-pipeline finalize at § 5.2 step 5 ships unchanged. Neither documented fallback (synthetic-thread injection / legacy REST `SubmitSummaryOnlyReviewAsync`) is needed.

**Implication for PR1+PR2:** Spec § 4 method set and § 5.2 step 5 stand as written. Plan Task 18 (the conditional C9 fallback `SubmitSummaryOnlyReviewAsync`) is **not triggered** — skip it.
...
- [x] **C9** — verified 2026-05-12 (S5 PR0a): GraphQL accepted the empty-threads Comment finalize (`state: COMMENTED`); empty-pipeline path ships unchanged, plan Task 18 not triggered. See § C9.
- [x] **Submit-pipeline GraphQL input/field shapes** — confirmed via schema introspection during S5 PR1 (2026-05-12): `AddPullRequestReviewInput.{pullRequestId, commitOID, body, event, comments, threads}`; `AddPullRequestReviewThreadInput.{path, body, pullRequestId, pullRequestReviewId, line, side, startLine, startSide, subjectType}`; `AddPullRequestReviewThreadReplyInput.{pullRequestReviewId, pullRequestReviewThreadId, body}`; `SubmitPullRequestReviewInput.{pullRequestId, pullRequestReviewId, event, body}`; `DeletePullRequestReviewInput.{pullRequestReviewId}`. **There is no `deletePullRequestReviewThread` mutation** — the only delete mutations are `deletePullRequestReview` and `deletePullRequestReviewComment(input:{id})`; `DeletePendingReviewThreadAsync` is therefore implemented as a `node(id:)` thread-comments lookup + per-comment `deletePullRequestReviewComment` (a thread vanishes once its last comment is deleted). `FindOwnPendingReviewAsync`'s query fields all exist: `PullRequest.reviews(first, states, author)`, `PullRequestReview.{viewerDidAuthor, commit{oid}, createdAt, comments}`, `PullRequest.reviewThreads`, `PullRequestReviewThread.{path, line, diffSide, originalLine, isResolved, comments}`, `PullRequestReviewComment.{id, body, originalCommit{oid}, pullRequestReview{id}}`. See the deferrals sidecar "[Decision] `DeletePendingReviewThreadAsync` is implemented via comment-deletes".

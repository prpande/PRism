...
| M22 | `react-markdown` v9 strict-mode sanitization is on by default and tight enough for untrusted PR content | **PARTIAL** (claim was wrong: posture is *escaping by default*, not sanitization; XSS surface is still closed but for the wrong reason) | Add an *escaping* regression test (XSS payloads → assert no `script` / `onerror` / `javascript:` survive *as escaped text*); document `rehype-raw` + `rehype-sanitize` pairing as the prerequisite for any future raw-HTML feature. |
...
1. **Escaping regression test (the load-bearing item).** Add a vitest test that feeds a known-malicious markdown payload (e.g., `<img src=x onerror="alert(1)">`, `<a href="javascript:alert(1)">click</a>`, `<script>alert(1)</script>`, `[ok](javascript:alert(1))`) to the rendering component and asserts that:
   - Raw `<img>`, `<script>`, `<iframe>` tags appear as **escaped text** in the DOM (no real `<script>` element, no `onerror` attribute that ever existed).
   - `[text](javascript:...)` links are stripped or transformed by the explicit `urlTransform` allowlist (`https?` / `mailto` only — already in `MarkdownRenderer.tsx`).
   - Absent `rehype-raw` / `rehype-sanitize` from the imports.
   - The test is what guarantees a future "loosen rendering to support feature X" change cannot silently flip the posture from "escaped" to "parsed-but-not-sanitized."
2. **`rehype-raw` + `rehype-sanitize` pairing as the prerequisite for any future raw-HTML feature.** Anyone wanting to support `<details>`/`<summary>` (or any other element teams' PR templates use) must add **both** `rehype-raw` (to surface HTML nodes) **and** `rehype-sanitize` (to filter them) in the same change. Adding only `rehype-raw` would parse arbitrary HTML and ship it to the DOM — an immediate XSS surface. The doc must call out this pairing rule before the first such change lands.

The escaping regression test is the load-bearing trip-wire; the pairing rule is the doc-side defense against silent posture changes. Both are P3-class polish in S3's deferrals; M22 records them so future contributors know the trip-wire is missing and what the actual rendering posture is.

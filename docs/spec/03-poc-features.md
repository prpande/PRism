...
A dedicated Settings page (route `/settings`, third tab in the app header) is the canonical surface for user preferences. Shipped in S6; see [`../specs/2026-05-15-s6-polish-and-distribution-design.md`](../specs/2026-05-15-s6-polish-and-distribution-design.md) § 2 for the full design. Four sections: **Appearance** (theme, accent, AI preview — same controls as the header pop-out, plus the comprehensive surface), **Inbox sections** (per-section visibility toggles), **Connection** (GitHub host, config-file path with copy, logs path with copy), **Auth** (Replace token link). Saves are optimistic with rollback-on-error and a `'success'` toast that auto-dismisses.
...
The **Replace token** link in the Auth section drives the Setup-screen re-prompt with the S6 identity-change rule: same-login replace preserves all state; different-login replace preserves drafts and clears the GraphQL Node IDs owned by the prior login (foreign-pending-review modal handles the orphan cleanup on next submit).

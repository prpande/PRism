...
- Storage: `state.json.reviewSessions[ref].viewedFiles[<filePath>] = <headShaAtTimeOfMark>` (C# field: `ReviewSessionState.ViewedFiles`; on-disk JSON key after the kebab policy applies: `viewed-files`) — see `02-architecture.md` § "State schema (PoC)."

...
describe('AiFailureToast timeout copy', () => {
  ...
  it('shows timeout copy + Adjust-timeout deep-link when timed out', async () => {
    ...
    // also contains "timed out", so a loose regex would not distinguish the two branches. The timeout
    // copy now also lists the failed seams (claude[bot] #1); renderToast uses seams=['summary'].
    expect(screen.getByText('AI generation timed out: summary.')).toBeInTheDocument();

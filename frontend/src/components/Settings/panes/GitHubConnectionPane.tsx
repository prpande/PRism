  ...
  return (
    <section aria-labelledby="ghc-heading">
      <div className={pane.head}>
        <div>
          <h2 id="ghc-heading" className={pane.title}>
            GitHub Connection
          </h2>
          ...
          {inFlight ? (
            <>
              <Link
                to="/setup?replace=1"
                aria-disabled="true"
                aria-describedby="ghc-replace-help"
                title={tooltipMsg}
                onClick={(e) => e.preventDefault()}
                className={pane.linkDisabled}
              >
                Replace token
              </Link>
              <span id="ghc-replace-help" className="sr-only">
                {tooltipMsg}
              </span>
            </>
          ) : (
      ...
      <div className={pane.row}>
        ...
        <div className={pane.spring}>
          <code className={pane.mono}>{host}</code>
        </div>
      </div>
    </section>
  );

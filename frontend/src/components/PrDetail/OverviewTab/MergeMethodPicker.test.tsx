...
describe('allowedList / firstAllowed', () => {
  ...
  it('defaults to all three when none flagged', () => {
    expect(allowedList({ merge: false, squash: false, rebase: false })).toEqual([
      'merge',
      'squash',
      'rebase',
    ]);
...
describe('MergeMethodPicker', () => {
  it('renders a radiogroup of allowed methods only', () => {
    render(
      <MergeMethodPicker
        allowed={{ merge: true, squash: true, rebase: false }}
        value="merge"
        onChange={() => {}}
      />,
    );
  ...
  it('renders nothing when only one method is allowed', () => {
    const { container } = render(
      <MergeMethodPicker
        allowed={{ merge: false, squash: true, rebase: false }}
        value="squash"
        onChange={() => {}}
      />,
    );
  ...
  it('arrow key changes selection', async () => {
    ...
    render(
      <MergeMethodPicker
        allowed={{ merge: true, squash: true, rebase: true }}
        value="merge"
        onChange={onChange}
      />,
    );

...
public sealed class SystemCliProcessRunner : ICliProcessRunner
{
    public async Task<ProcessResult> RunAsync(ProcessSpec spec, CancellationToken ct)
    {
        ...
        // CA2025: `process` (whose StandardInput is IDisposable) is captured by writeTask. We DO
        // finish using it before disposal — DrainWriteAsync(writeTask) is awaited on EVERY return
        // path (timeout branch and normal branch) before `using var process` disposes. The
        // analyzer's flow analysis can't prove that across the branching, so suppress per docs.

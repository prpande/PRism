...
- **`[Defer]`** (will revisit): open a GitHub issue — label `deferred` (create the label once if absent), title the deferred work, and fill the body from the template below. The issue is the durable record; close it when the work lands. When starting a slice, find prior deferrals that target it by searching **both** surfaces: `gh issue list --label deferred --state open` (new work — run from inside the repo so it isn't pinned to one owner/fork) **and** a grep of the frozen `*-deferrals.md` sidecars under `docs/specs/` **and** `docs/plans/` (older work, not migrated — plan-side sidecars exist too, e.g. `docs/plans/2026-05-06-s3-pr-detail-read-deferrals.md`).
...
Issue body template for a `[Defer]`:

```markdown
**Reason:** <why deferred — one paragraph>
**Revisit-when:** <concrete trigger>
**Source:** <session — e.g., ce-doc-review pass on YYYY-MM-DD>
**Severity:** P0 | P1 | P2 | P3
**Original finding evidence:** <quote or paraphrase>
**Source doc:** [path/to/spec-or-plan.md](…)
```

...
PRism's V1 review experience is complete and shipping as unsigned pre-release builds (see the [Releases page](https://github.com/prpande/PRism/releases)); current work on `main` is fit-and-finish polish plus AI augmentation against the in-place seams. (The former long-lived `V2` "AI-development" branch was merged into `main` and deleted at the v2 cutover — PR #552; all work now lands on `main`. See [`v2-ai-effort.md`](v2-ai-effort.md).) Main contents:

- `PRism.sln` and seven backend projects: `PRism.Core`, `PRism.Core.Contracts`, `PRism.GitHub`, `PRism.Web`, `PRism.AI.Contracts`, `PRism.AI.Placeholder`, `PRism.AI.ClaudeCode`

...
// The terminal result of a SubmitPipeline.SubmitAsync run (spec § 5.1). Five variants:
...
//  - Cancelled: caller signaled cancellation mid-step (e.g., /submit/discard); pipeline returned
//    cleanly without trying further steps. Carries the last-Started SubmitStep so the endpoint's
//    SSE bridge can emit the terminal Failed for that step (and not strand the dialog in an
//    orphan "Started" state) plus a short Reason for the Information-level structured log.
[SuppressMessage("Design", "CA1034:Nested types should not be visible",
    Justification = "Closed result hierarchy: the five variants are intentionally namespaced under " +
                    "SubmitOutcome (SubmitOutcome.Success / .Failed / .ForeignPendingReviewPromptRequired / " +
                    ".StaleCommitOidRecreating / .Cancelled) — the canonical discriminated-union shape per " +
                    "spec § 5.1. Promoting them to top-level types would lose the union name from every call site.")]
